...
export function AdminCommissionsPage() {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      showToast({ tone: 'error', title: 'Falha ao carregar comissões', description: error.message || 'Tente novamente.' })
    }
  ...
  async function markAsPaid(closureId) {
    ...
    try {
      ...
      showToast({ tone: 'success', title: 'Comissão marcada como paga' })
      await reload()
    } catch (error) {
  ...
  return (
    <section className="space-y-5 pb-6">
      <PageHeader
        eyebrow="Admin"
        title="Comissões"
        description="Gestão completa da comissão gerada, paga e pendente."
        actions={
          <input
            type="month"
      ...
      <SummaryGrid columns={4}>
        <StatCard label="Comissão gerada no mês" value={formatCurrency(monthlySummary.gerada)} />
        <StatCard label="Comissão paga no mês" value={formatCurrency(monthlySummary.paga)} />
        <StatCard label="Comissão pendente no mês" value={formatCurrency(monthlySummary.pendente)} />
        <StatCard
          label="Pendentes da semana atual"
          value={pendingCount}
          hint={`${dayjs(currentWeek.startDate).format('DD/MM')} até ${dayjs(currentWeek.endDate).format('DD/MM')}`}
        />
      </SummaryGrid>

      <SectionCard
        title="Fechamento semanal atual"
        subtitle={`Ciclo fixo de comissão (${dayjs(currentWeek.startDate).format('DD/MM')} – ${dayjs(currentWeek.endDate).format('DD/MM')}).`}
      >
        {weeklyRows.length === 0 ? (
          <EmptyState title="Sem fechamento semanal" description="Não há comissão semanal registrada para este ciclo." />
        ) : (
          <DataTable
            columns={[
              { key: 'funcionario', label: 'Funcionário', render: (row) => row.usuario?.nome || 'Sem nome' },
      ...
      <SectionCard title="Comissão por funcionário no mês" subtitle="Separação de gerada, paga e pendente por colaborador.">
        <DataTable
          columns={[
          ...
          rows={employeeRows}
          empty="Sem comissões de funcionários no mês selecionado."
        />
      </SectionCard>

      <SectionCard title="Histórico de pagamentos" subtitle="Rastreabilidade de pagamentos marcados no fechamento semanal.">
        <DataTable
          columns={[
            { key: 'funcionario', label: 'Funcionário', render: (row) => row.usuario?.nome || 'Sem nome' },
            {
              key: 'periodo',
              label: 'Período pago',
              render: (row) => `${dayjs(row.semana_inicio).format('DD/MM/YYYY')} até ${dayjs(row.semana_fim).format('DD/MM/YYYY')}`,
            },
